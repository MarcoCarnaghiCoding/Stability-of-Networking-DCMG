% This Script is to generate the simulations for the plot of: Tau_max vs Kii vs Kpi

% Full version:
%               Diferent plot are going to be defined for different 
%              controllers gains configurations.

%README:
%       The idea is to make 2 nested loops:
%           * Outer loop: takes a value of P_eq = P_eq_i.
%           * Mid loop: Takes different valued for Kpi
%           * Inner loop: 
%                        * Takes values of Tau for a constant Kii, d(tau)/dt and P_eq_i
...
for k_iv_1=1:10:101
    ...
    for k_pi_1=0.5:0.5:50
        ...
        for k_ii_1=1:1:100
            ...
            mu  = 0.5 ;
            status2008 = 'Solved';
            while status2008(1:6) == 'Solved'

                %-----------------------------------------------------------------
                % UPDATE REGISTER       
                %-----------------------------------------------------------------
                register(iteration,:) = [k_iv_1,k_pi_1,k_ii_1,tau,mu];
                tau = tau + 0.01;
                h   = tau; tau_max = tau;     
                if k_ii_1 < k_pi_1
                    status2008 = '';
